...
export function formatEventDate(ev: any) {
...
export function monthKeyFromEvent(ev: any) {
  const d = formatEventDate(ev);
  return d ? d.slice(0, 7) : "";
}
export function groupByMonth(events: any[]) {
...
export async function listEventsForYear(year: number): Promise<any[]> {
